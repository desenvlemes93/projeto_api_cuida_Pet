import 'package:ProjetoAPICuidaPet/application/database/database_connection.dart';
import 'package:ProjetoAPICuidaPet/application/database/i_database_connection.dart';
import 'package:ProjetoAPICuidaPet/application/exceptions/database_exception.dart';
...
import 'package:ProjetoAPICuidaPet/application/helpers/cripty_helper.dart';
...
''';
      ...
      if (e.message.contains('usario.email_UNIQUE')) {
        log.error('Usuario ja cadastrada no banco de dados');
...
''';
      ...
      if (result.isEmpty) {
        ...
        throw UserNotfoundException(message: 'Usuário ou senha invalidos');
      } else {
      ...
      log.error('Ero ao realizar login');
...
''',
 ...
 ''';
      ...
      log.error('Erro ao confirmar logins', e, s);
...
''',
      ...
      if (result.isEmpty) {
        throw UserNotfoundException(message: 'Usuario não encontrado id ${id}');
      } else {
        final dataMysql = result.first;
        return User(
          id: dataMysql['id'] as int,
  ...
  Future<void> updateDeviceToken(
    ...
    try {
      ...
      log.error('erro ao atualizar o device ');
      throw DatabaseException();
    } finally {
      await conn?.close();
    }
